...
def information_extract(path):
    ...
    def Basic_details(text):
        dict_basic =  []
        df_basic = pd.read_csv('/home/amit/SIH/Backend/report/Lab Reports/Basic_details.csv')
    ...
    def Blood_Test_Details(text):
        dict_blood =  []
        df = pd.read_csv('/home/amit/SIH/Backend/report/Lab Reports/BLOOD TEST.csv')
    ...
    def Urine_Test_Details(text):
        dict_urine =  []
        df = pd.read_csv('/home/amit/SIH/Backend/report/Lab Reports/URINE_TEST - Sheet1.csv')
        for i in range(len(df)):
            param = df['Fields'][i]
    ...
    def Liver_Test_Details(text):
        dict_liver =  []
        df = pd.read_csv('/home/amit/SIH/Backend/report/Lab Reports/LIVER FUNCTION PROFILE,SERUM - Sheet1.csv')
        for i in range(len(df)):
            param = df['Fields'][i]
            match = re.search(param+' (\S+)', text)
            if match:
    ...
    def Stool_Test_Details(text):
        dict_stool =  []
        df = pd.read_csv('/home/amit/SIH/Backend/report/Lab Reports/STOOL_TEST - Sheet1.csv')
        for i in range(len(df)):
            param = df['Fields'][i]
            match = re.search(param+' (\S+)', text)
            if match:
    ...
    os.system("/home/amit/CliNER/cliner predict --txt " + arg1 + " --out /home/amit/CliNER/data/predictions --model /home/amit/CliNER/models/silver.crf.1 --format i2b2")
